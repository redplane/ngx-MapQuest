...
import { MQ_HEAT_LAYER_REQUIRED_FILES } from '../../constants/injectors';
...
export class MqHeatLayerDirective implements OnInit, OnDestroy {
  ...
  public constructor(
    ...
    @Inject(MQ_HEAT_LAYER_REQUIRED_FILES)
    @Optional()
    protected readonly mqMapHeatFiles: MqSystemFile[],
    protected readonly mqFileLoaderService: MqFileLoaderService
  ) {
    this._uuid = uuid();

    this._updateInstanceSubject = new Subject<{
      property: HEAT_LAYER_PROPERTY;
      value: any;
    }>();
    ...
    const hookMapLoadedSubscription = this.mqMapService.mapLoadedEvent
      .pipe(
        ...
        switchMap((mapControl) => {
          ...
          return this.mqFileLoaderService
            .loadSystemFilesAsync(requiredFiles)
            .pipe(map(() => mapControl));
        })
      )
