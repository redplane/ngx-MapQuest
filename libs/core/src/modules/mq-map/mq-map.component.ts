...
export class MqMapComponent implements OnInit, AfterViewInit, AfterContentInit {
  ...
  public readonly baseLayerChange: EventEmitter<LayersControlEvent> =
    new EventEmitter<LayersControlEvent>();
  ...
  @Output()
  public readonly initializedEvent: EventEmitter<MqMapComponent> =
    new EventEmitter<MqMapComponent>();
  ...
  public constructor(
    ...
    @Inject(MAP_QUEST_KEY_RESOLVER)
    protected mapKeyResolver: IMqMapKeyResolver,
    @Inject(DOCUMENT) protected readonly document: Document,
    protected readonly mqMapService: MqMapService,
    @Inject(MQ_MAP_REQUIRED_FILES)
    @Optional()
    protected readonly mqSystemFiles: MqSystemFile[],
  ...
  public ngOnInit(): void {
    const loadSystemFilesSubscription = this.loadSystemFilesAsync().subscribe(
      () => {
          ...
          .pipe(
            switchMap(() => {
              ...
              if (
                !this._key &&
                this.mapKeyResolver &&
                this.mapKeyResolver.getMapQuestKeyAsync
              ) {
                getKeyObservable = this.mapKeyResolver.getMapQuestKeyAsync();
              }

              return getKeyObservable.pipe(
                tap((key) => {
                  this._key = key;
                  ...
                  const mapControl = L.mapquest.map(
                    this.elementRef.nativeElement,
                    options
                  );

                  mapControl.addEventListener(
                    'baselayerchange',
                    (event: LayersControlEvent) =>
                      this.baseLayerChange.emit(event)
                  );
                  mapControl.addEventListener(
                    'overlayadd',
                    (event: LayersControlEvent) => this.overlayAdd.emit(event)
                  );
                  mapControl.addEventListener(
                    'overlayremove',
                    (event: LayersControlEvent) =>
                      this.overlayRemove.emit(event)
                  );
                  mapControl.addEventListener('layeradd', (event: LayerEvent) =>
                    this.layerAdd.emit(event)
                  );
                  mapControl.addEventListener(
                    'layerremove',
                    (event: LayerEvent) => this.layerRemove.emit(event)
                  );
                  mapControl.addEventListener(
                    'zoomlevelschange',
                    (event: Event) => this.zoomLevelsChange.emit(event)
                  );
                  mapControl.addEventListener('resize', (event: ResizeEvent) =>
                    this.resize.emit(event)
                  );
                  mapControl.addEventListener('unload', (event: Event) =>
                    this.unload.emit(event)
                  );
                  mapControl.addEventListener('viewreset', (event: Event) =>
                    this.viewReset.emit(event)
                  );
                  mapControl.addEventListener('load', (event: Event) =>
                    this.loadEvent.emit(event)
                  );
                  mapControl.addEventListener('zoomstart', (event: Event) =>
                    this.zoomStart.emit(event)
                  );
                  mapControl.addEventListener('movestart', (event: Event) =>
                    this.moveStart.emit(event)
                  );
                  mapControl.addEventListener('zoom', (event: Event) =>
                    this.zoom.emit(event)
                  );
                  mapControl.addEventListener('move', (event: Event) =>
                    this.move.emit(event)
                  );
                  mapControl.addEventListener('zoomend', (event: Event) =>
                    this.zoomEnd.emit(event)
                  );
                  mapControl.addEventListener('moveend', (event: Event) =>
