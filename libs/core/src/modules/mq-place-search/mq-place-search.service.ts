...
import { MAP_QUEST_KEY_RESOLVER } from '../../constants/injectors';
...
export class MqPlaceSearchService implements IMqPlaceSearchService {
  ...
  public constructor(injector: Injector) {
    this._mqMapKeyResolver = injector.get(MAP_QUEST_KEY_RESOLVER);
  ...
  public doPlaceSearchAsync(
    model: MqPlaceSearchRequest
  ): Observable<MqPlaceSearchResponse> {
    return this._mqMapKeyResolver.getMapQuestKeyAsync().pipe(
      mergeMap((apiKey: string) => {
        const apiUrl = `https://www.mapquestapi.com/search/v4/place`;
        let httpParams = new HttpParams().append('key', apiKey);
        ...
        httpParams = httpParams
          .append('pageSize', `${model.pageSize}`)
