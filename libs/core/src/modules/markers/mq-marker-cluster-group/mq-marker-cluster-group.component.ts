...
import { MQ_MARKER_CLUSTER_GROUP_REQUIRED_FILES } from '../../../constants';
...
export class MqMarkerClusterGroupComponent implements OnInit, OnDestroy {
  ...
  public constructor(
    @Optional()
    @Inject(MQ_MARKER_CLUSTER_GROUP_REQUIRED_FILES)
  ...
  public ngOnInit(): void {
    ...
    const hookMapReadyEventSubscription = this.mqMapService.mapLoadedEvent
  ...
  protected buildMarkerClusterGroupSubject(): void {
    ...
    this._buildMarkerClusterGroupSubscription =
      this._buildMarkerClusterGroupSubject
        .pipe(switchMap(() => this.loadSystemFilesAsync()))
        .subscribe(() => {
